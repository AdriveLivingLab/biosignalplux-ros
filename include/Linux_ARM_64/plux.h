/**
 * \file
 * \copyright  Copyright 2015-2021 PLUX - Wireless Biosignals, S.A.
 * \version    1.8
 * \date       December 2019
...
#define PLUX_API_VERSION      "1.11"                        ///< Current API version string
#define PLUX_API_BUILD_DATE   __DATE__                      ///< Compile API date
//const char *PLUX_API_BUILD_DATE = __DATE__ " "  __TIME__; ///< Compile API timedate
...
namespace Plux
{
   ...
   struct Clock
   {
      ...
      enum Source
      {
         ...
         Bluetooth,  ///< Bluetooth piconet clock
         /// \cond
         //DigAccel,
         /// \endcond
      };
   ...
   struct Event
   {
      ...
      enum Type
      {
         ...
         TypeSensorTrack = 11,      ///< %Event is a EvtSensorTrack object.
         TypeFrameData = 12,        ///< %Event is a EvtFrameData object.
         TypeDeviceState = 13,      ///< %Event is a EvtDeviceState object.
         TypeCalibrationData = 14,  ///< %Event is a EvtCalibrationData object.

         
         // internal use only
         /// \cond
         TypeTest = 0x70,           ///< %Event is a EvtTest object.
         /// \endcond
      };
   ...
   struct EvtSensorTrack : public Event
   {
       int port;        ///< Sensor port.
       bool connect;    ///< True if sensor was connected, false otherwise.
       bool idChanged;  ///< True if a different sensor was connected on the port, according to port-sensor map defined at acquisition start.
   };

   // Acquisition send data frames event class
   struct EvtFrameData : public Event
   {
      int len;
      const void *data;
   };
   ...
   struct EvtDeviceState : public Event
   {
   ...
   // Calibration send data frames event class
   struct EvtCalibrationData : public Event
   {
      int len;
      const void *data;
   };
   ...
   public:
      ...
      enum ProductId
      {
         PID_BioPlux           = 0x0101,   ///< BioPlux device (PID: 1.01)
         PID_BiosignalsPlux    = 0x0201,   ///< biosignalsplux device (PID: 2.01)
         PID_BiosignalsPluxPRO = 0x0205,   ///< biosignalsplux PRO+ device (PID: 2.05)
         PID_MotionPlux        = 0x020A,   ///< Motionplux (PID: 2.10)
         PID_OpenBan           = 0x0214,   ///< OpenBan device (PID: 2.20)
         PID_OpenBanV2_DDME    = 0x0215,   ///< OpenBan v2 device - DDME (PID: 2.21)
         PID_OpenBanV2         = 0x0216,   ///< OpenBan v2 device (PID: 2.22)
         PID_fNIRS             = 0x021E,   ///< fNIRS device (PID: 2.30)
         PID_StimPlux          = 0x0301,   ///< StimPlux device (PID: 3.01)
         PID_MuscleBan         = 0x0502,   ///< MuscleBan device (PID: 5.02)
         PID_BITalino          = 0x0601,   ///< Original BITalino device (PID: 6.01)
         PID_BITalinoRev       = 0x0602,   ///< BITalino (r)evolution device (PID: 6.02)
         PID_scoliosis         = 0x0701,   ///< EastCoast Scoliosis (PID: 7.01)
         PID_MuscleBanWearable = 0x0801,   ///< MuscleBan device v2 - Wearable version with SD card (PID: 8.01)
         PID_CardioBanWearable = 0x0802,   ///< CardioBan device - Wearable version with SD card (PID: 8.02)
         PID_RespiBanWearable  = 0x0803,   ///< RespiBan device - Wearable version with SD card (PID: 8.03)
      };
       ...
       */
#ifdef __linux__
      BaseDev(const String &path, const String &bindpath = "");
#else
   ...
   struct Sensor
   {
      ...
      uint64_t    uid;             ///< %Sensor 48-bit unique identifier
      Class       clas;            ///< %Sensor class
      int         serialNum;       ///< %Sensor serial number within its class
      int         hwVersion;       ///< %Sensor hardware version number
      int         type;            ///< %Sensor interface type bitmask
      time_t      productionTime;  ///< %Sensor production timestamp
      Color       color;           ///< %Sensor sleeve color
      Properties  characteristics; ///< %Sensor characteristics. See [Sensor properties](contents/sensor_properties.md)
      Properties  measurCalib;     ///< %Sensor measurements or calibration data. See [Sensor properties](contents/sensor_properties.md)
   };
   ...
   /** Acquisition source class.
    An acquisition source is an external sensor connected to a device port or a device internal sensor.
   ...
   public:
       ...
       */
#ifdef __linux__
      SignalsDev(const String &path, const String &bindpath = "");
#else
   ...
   public:
       ...
       */
#ifdef __linux__
      MemoryDev(const String &path, const String &bindpath = "");
#else
   ...
   private:
      /** Returns all session schedules stored on the device.
       ...
       \remarks On current devices firmware, only one schedule can be stored.
       */
      int  getSchedules_ext(Schedules &schs);
      ...
      /** Adds a session schedule to the device.
       ...
       \remarks On current devices firmware, only one schedule can be stored.
       */
      void addSchedule_ext(const Schedule &sch);
   };
